...
### 当前主要功能
- **课表与日程**：导入课表、按周查看、课程详情、自定义壁纸；冲突时段合并显示为"课程冲突"卡片，有 18:30 后的课程时给出"还有更多课程"提示；打开应用时自动刷新课表（10 分钟下限）并按课程粒度合并改动——时间/地点/教师调整与新增课次及时采纳，整门缺失或课次减少的课程保留本地，不被出问题的课表源覆盖，变更经课表页提醒与详情弹窗展示，持续缺失或差异过大时挂起待确认；教务刷新会保留账号隔离的版本快照，可随时恢复；支持 ICS 课表导入/导出
